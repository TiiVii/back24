...
const findEntryById = async (id, userId) => {
  try {
    const [rows] = await promisePool.query(
      'SELECT * FROM FeelingEntries WHERE entry_id = ? AND user_id = ?',
      [id, userId],
    );
    return rows[0];
  } catch (e) {
...
const addEntry = async (entry) => {
  const {user_id, entry_date, mood, crying, breakdowns, notes} = entry;
...
const updateEntryById = async (entryId, userId, entryData) => {
  try {
    ...
    const sql = promisePool.format(
      `UPDATE FeelingEntries SET ? WHERE entry_id=? and user_id=?`, params,
    );
